...
/**
 * A self-resizing flat array of floats designed for use within the
 ...
 *
 * <p>
 * The {@code FloatList} automatically grows to accommodate additional elements
 ...
 * resizing of float-based components.
 * </p>
 *
 * <p>
 ...
 * <h2>Usage Example</h2>
 * <pre><code>
 * FloatList entityValues = new FloatList();
 * entityValues.add(5.0f);
 * entityValues.add(10.0f);
 * float firstValue = entityValues.get(0); // Returns 5.0f
 * </code></pre>
 *
 * <h2>Resizing Behaviour</h2>
 * <p>
 * The list increases by 1.5x when its capacity is full and contracts to half
 ...
 * {@code FloatList} concurrently and at least one thread modifies the list,
...
public final class FloatList {
     ...
     * 
     * <p>
     * This method retrieves the float stored at the given index, providing 
     ...
     * can hold before it needs to grow. The capacity refers to the length of
     * the internal array and is distinct from the size of the list, which
     * represents the number of elements actually stored in the list.
     * </p>
     *
     ...
     */
    public boolean removeAll(float[] elements) {
    ...
    public boolean removeElement(float element) {
        for (int index = 0; index < size; index++) {
            if (Float.compare(data[index], element) == 0) {
                remove(index);
                return true;
            }
        }
    ...
    public int indexOf(float element) {
        for (int i = 0; i < size; i++) {
            if (Float.compare(data[i], element) == 0) {
                return i;
            }
        }
     ...
     * brackets (e.g., {@code [1.0, 2.0, 3.0]}). The string includes only the elements 
    ...
    private String outOfBoundsMessage(int index, boolean insertion) {
        String str = insertion ? "above" : "equal to or above";
        return String.format("Index %d cannot be negative, or %s the current size of the FloatList (%d)", index, str, size);
    }
