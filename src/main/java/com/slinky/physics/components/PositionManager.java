...
/**
 * {@code PositionManager} is a specialised extension of {@link VectorStorage}
 * designed to handle the storage and management of position vectors for
 ...
 * but it represents a meaningful abstraction for managing position-related data
 * within the system.
 *
 * <p>
 * The class enforces an "is-a" relationship, where {@code PositionManager}
 * operates identically to {@code VectorStorage}, but is explicitly named for
 * handling position vectors. This ensures semantic clarity and type safety when
 * differentiating between position, velocity, force, or other similar vector
 * data in the ECS system.</p>
 *
 * <h2>Use Case</h2>
 * <p>
 * {@code PositionManager} is used in systems that require
 * managing and processing the positional data of entities. For example, it can
 * store and update the coordinates of an entity in a 2D or 3D physics
 * simulation. By using {@code PositionManager}, developers can avoid mixing up
 * position vectors with other types of vector data like velocity or force,
 ...
 */
public final class PositionManager extends VectorStorage {

    // =============================[ Constructors ]============================= \\
    /**
     * Constructs a {@code PositionStorage} with the specified initial capacity
     ...
     */
    public PositionManager(EntityManager entityManager, int initialEntityCapacity, int maxEntityCapacity) {
        super(Component.POSITION, entityManager, initialEntityCapacity, maxEntityCapacity);
    }
